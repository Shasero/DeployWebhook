...
if [[ -z $PROJECT_NAME || -z $DOMAIN_NAME || -z $TOKEN || -z $EMAIL_SSL ]]; then
    echo "=== Нет необходимых переменных окружения (PROJECT_NAME, DOMAIN_NAME, TOKEN, EMAIL_SSL) ! ==="
...
echo "=== Проверка состояния ТГ вебхука: https://api.telegram.org/bot$TOKEN/getWebhookInfo ==="
