...
sudo cp -R /root/DeployWebhook/Scripts ./
sudo cp -R /root/DeployWebhook/Service ./
sudo cp -R /root/DeployWebhook/Nginx ./
sudo cp -R /root/DeployWebhook/html_for_bot ./
sudo cp /root/DeployWebhook/main.py ./
...
echo
read -p "=== Введите токен телеграмм бота: " token
echo
